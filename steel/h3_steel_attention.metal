// Flattened from ml-explore/mlx (MIT), commit b6368984 plus the head_dim-96
// unroll change: mlx/backend/metal/kernels/steel/{defines,utils}.h,
// steel/utils/{type_traits,integral_constant}.h, steel/attn/{params,transforms,nax}.h
// and steel/attn/kernels/steel_attention_nax.h. Only the NAX (M5 TensorOps)
// full-attention kernel is kept. Includes of mlx headers are inlined; the
// system includes are hoisted here.
...
// From mlx/backend/metal/kernels/utils.h: the parts the kernel needs.
template <typename U>
struct Limits {
  ...
  for (int kb = 0; kb < kb_lim; kb++) {
    ...
    for (short iq = 0; iq < TQ; iq++) {
      ...
      for (short ik = 0; ik < TK; ik += 2) {
        ...
        // for head dim 128. Head dim 96 (TD = 6) is the exception: a count of
        // 4 leaves a two-iteration remainder loop, and unrolling it fully
        // measured 31 -> 42 TFLOPS at N=14700 on M5 Max.
#pragma clang loop unroll_count(TD == 6 ? 6 : 4)
...
// ===== instantiations for h3-metal =====
#define h3_instantiate_attn(tname, dtype, bq, bk, bd, wm, wn)                 \
  template [[host_name("h3_steel_attention_" #tname "_bq" #bq "_bk" #bk      \
                       "_bd" #bd "_wm" #wm "_wn" #wn)]] [[kernel]]            \
  decltype(attention_nax<dtype, bq, bk, bd, wm, wn, float, float>)           \
  attention_nax<dtype, bq, bk, bd, wm, wn, float, float>;
h3_instantiate_attn(bfloat16, bfloat16_t, 64, 32, 96, 4, 1)
h3_instantiate_attn(bfloat16, bfloat16_t, 64, 32, 64, 4, 1)
h3_instantiate_attn(bfloat16, bfloat16_t, 64, 32, 128, 4, 1)
